...
```
Nel grafico mostrato sopra possiamo dare una migliore interpetazione di quelli che sono i coefficienti di correlazione di alcune variabili già viste in precedenza, unite alla loro densità di kernel, mostrata sulla diagonale principale. Nella parte triangolare inferiore della matrice possiamo identificare gli scatterplot unito ad con  fit di un modello lineare.
...
```{r echo=TRUE, message=FALSE, warning=FALSE}
# Diminuisco il campione e filtro i terremoti di origine naturale
terremoti_sampled <- terremoti %>%
  group_by(type) %>%
  filter(!(type == "earthquake" & runif(n()) < 0.85))
...
ggplot() +
  ...
  geom_point(data = terremoti_sampled, aes(x = longitude, y = latitude, color = type, size = mag), alpha = 0.3) +
...
ggplot(data = frequenza_terremoti_minori, aes(x = reorder(type, Percentuale),
  ...
  geom_text(aes(label = round(Percentuale, 1)), hjust = -0.2,
            color = "#2C3E50", size = 5)
```
```{r echo=TRUE, message=FALSE, warning=FALSE}
...
ggplot(max_magnitudes, 
  ...
  scale_y_discrete(labels = function(x) ifelse(tolower(x) == "usa", "USA", str_to_title(x))) +
  labs (x = "MAG",
        y = "",
